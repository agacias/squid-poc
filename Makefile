...
build:
        . ./env
        docker build -f Dockerfile-squid --tag ${REGISTRY}:5000/${APPLICATION}:${TAG} --build-arg VCS_REF=`git rev-parse --short HEAD` --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` .
